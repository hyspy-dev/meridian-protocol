...
public class PlayEmote implements Packet, ToServerPacket {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("PlayEmote", offset, (int) mem.byteSize());
        long needed = (long) offset + 1;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("PlayEmote", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static String getEmoteId(MemorySegment mem, int offset) {
        return hasEmoteId(mem, offset) ? PacketIO.readVarString("EmoteId", mem, offset + 1, 4096000): null;
    }
    ...
    /**
     * Decodes one PlayEmote and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static PlayEmote toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var varBase = offset + 1;
        var varPos = 0;
        String v0 = null;
        if (hasEmoteId(mem, offset)) {
            var off = varBase + varPos;
            var sp = VarInt.getWithLength(mem, off);
            v0 = PacketIO.readVarString("EmoteId", mem, off, 0, 4096000, sp);
            varPos += (int) sp + (int) (sp >>> 32);
        }
        var result = new PlayEmote(
            v0
        );
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        byte nullBits;
        nullBits = 0;
        if (this.emoteId != null) nullBits |= 0x01;
    ...
    public int computeSize() {
        int size = 1;
        if (emoteId != null) size += PacketIO.stringSize(emoteId);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PlayEmote other)) return false;
        return java.util.Objects.equals(this.emoteId, other.emoteId);
    }
