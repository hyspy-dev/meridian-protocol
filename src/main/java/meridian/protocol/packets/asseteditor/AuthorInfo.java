...
public class AuthorInfo {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("AuthorInfo", offset, (int) mem.byteSize());
        long needed = (long) offset + 13;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("AuthorInfo", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static String getName(MemorySegment mem, int offset) {
        return hasName(mem, offset) ? PacketIO.readVarString("Name", mem, offset + getValidatedOffset(mem, offset, 1, 13, "Name"), 4096000): null;
    }
    ...
    public static String getEmail(MemorySegment mem, int offset) {
        return hasEmail(mem, offset) ? PacketIO.readVarString("Email", mem, offset + getValidatedOffset(mem, offset, 5, 13, "Email"), 4096000): null;
    }
    ...
    public static String getUrl(MemorySegment mem, int offset) {
        return hasUrl(mem, offset) ? PacketIO.readVarString("Url", mem, offset + getValidatedOffset(mem, offset, 9, 13, "Url"), 4096000): null;
    }
    ...
    /**
     * Decodes one AuthorInfo and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static AuthorInfo toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        String v1 = null;
        if (hasEmail(mem, offset)) {
            requireSlot(mem, offset + 5, varPos, "Email");
            var off = varBase + varPos;
            var sp = VarInt.getWithLength(mem, off);
            v1 = PacketIO.readVarString("Email", mem, off, 0, 4096000, sp);
            varPos += (int) sp + (int) (sp >>> 32);
        } else {
            requireSlot(mem, offset + 5, -1, "Email");
        }
        
        String v2 = null;
        if (hasUrl(mem, offset)) {
            requireSlot(mem, offset + 9, varPos, "Url");
            var off = varBase + varPos;
            var sp = VarInt.getWithLength(mem, off);
            v2 = PacketIO.readVarString("Url", mem, off, 0, 4096000, sp);
            varPos += (int) sp + (int) (sp >>> 32);
        } else {
            requireSlot(mem, offset + 9, -1, "Url");
        }
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        if (this.email != null) nullBits |= 0x02;
        if (this.url != null) nullBits |= 0x04;
    ...
    if (email != null) size += PacketIO.stringSize(email);
    if (url != null) size += PacketIO.stringSize(url);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof AuthorInfo other)) return false;
        return java.util.Objects.equals(this.name, other.name) && java.util.Objects.equals(this.email, other.email) && java.util.Objects.equals(this.url, other.url);
    }
