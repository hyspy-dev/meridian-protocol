...
public class AssetEditorRequestChildrenList implements Packet, ToServerPacket {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("AssetEditorRequestChildrenList", offset, (int) mem.byteSize());
        long needed = (long) offset + 1;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("AssetEditorRequestChildrenList", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    /**
     * Decodes one AssetEditorRequestChildrenList and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static AssetEditorRequestChildrenList toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        var result = new AssetEditorRequestChildrenList(
            v0
        );
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof AssetEditorRequestChildrenList other)) return false;
