...
public class SuccessReply implements Packet, ToServerPacket, ToClientPacket {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("SuccessReply", offset, (int) mem.byteSize());
        long needed = (long) offset + 5;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("SuccessReply", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    /**
     * Decodes one SuccessReply and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static SuccessReply toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof SuccessReply other)) return false;
