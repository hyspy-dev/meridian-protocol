...
public class AssetEditorDiscardChanges implements Packet, ToServerPacket {
    ...
    @Nullable public TimestampedAssetReference[] assets;
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("AssetEditorDiscardChanges", offset, (int) mem.byteSize());
        long needed = (long) offset + 1;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("AssetEditorDiscardChanges", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static TimestampedAssetReference[] getAssets(MemorySegment mem, int offset) {
        ...
        if (off + lenOffset + (long) len * 9 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Assets", (int) java.lang.Math.min(off + lenOffset + (long) len * 9, Integer.MAX_VALUE), (int) mem.byteSize());
        off += lenOffset;
        var data = new TimestampedAssetReference[len];
        for (var i = 0; i < len; i++) {
            data[i] = TimestampedAssetReference.toObject(mem, off, walkCursor);
    ...
    public static AssetEditorDiscardChanges toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static AssetEditorDiscardChanges toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one AssetEditorDiscardChanges and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static AssetEditorDiscardChanges toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var varBase = offset + 1;
        var varPos = 0;
        var walkCursor = cursor != null ? cursor : new ReadCursor();
        TimestampedAssetReference[] v0 = null;
        if (hasAssets(mem, offset)) {
            ...
            if (off + lenOffset + (long) len * 9 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Assets", (int) java.lang.Math.min(off + lenOffset + (long) len * 9, Integer.MAX_VALUE), (int) mem.byteSize());
            off += lenOffset;
            v0 = new TimestampedAssetReference[len];
            for (var i = 0; i < len; i++) {
                v0[i] = TimestampedAssetReference.toObject(mem, off, walkCursor);
                off = walkCursor.position;
            }
            varPos = off - varBase;
        }
        var result = new AssetEditorDiscardChanges(
            v0
        );
...
size += VarInt.size(assets.length) + assetsSize;
    ...
    public AssetEditorDiscardChanges clone() {
        ...
        copy.assets = this.assets != null ? java.util.Arrays.stream(this.assets).map(e -> e.clone()).toArray(TimestampedAssetReference[]::new) : null;
        return copy;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof AssetEditorDiscardChanges other)) return false;
