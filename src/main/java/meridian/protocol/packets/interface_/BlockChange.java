...
public class BlockChange {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("BlockChange", offset, (int) mem.byteSize());
        long needed = (long) offset + 17;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("BlockChange", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static int getBlock(MemorySegment mem) {
        return getBlock(mem, 0);
    }
    
    public static int getBlock(MemorySegment mem, int offset) {
    ...
    public static BlockChange toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static BlockChange toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one BlockChange and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static BlockChange toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var result = new BlockChange(
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        mem.set(PacketIO.PROTO_INT, offset + 12, this.block);
        mem.set(PacketIO.PROTO_BYTE, offset + 16, this.rotation);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof BlockChange other)) return false;
        return this.x == other.x && this.y == other.y && this.z == other.z && this.block == other.block && this.rotation == other.rotation;
    }
