...
public class UpdateLanguage implements Packet, ToServerPacket {
    ...
    public static String getLanguage(MemorySegment mem, int offset) {
        return hasLanguage(mem, offset) ? PacketIO.readVarString("Language", mem, offset + 1, 4096000): null;
    }
    
    public static boolean hasLanguage(MemorySegment mem, int offset) {
    ...
    public static UpdateLanguage toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        var varPos = 0;
        String v0 = null;
        if (hasLanguage(mem, offset)) {
            var off = varBase + varPos;
            var sp = VarInt.getWithLength(mem, off);
            v0 = PacketIO.readVarString("Language", mem, off, 0, 4096000, sp);
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        byte nullBits;
        nullBits = 0;
        if (this.language != null) nullBits |= 0x01;
        ...
        if (this.language != null) {
            ...
            varOffset += PacketIO.writeVarString(mem, varOffset, this.language, 4096000);
        }
    ...
    public int computeSize() {
        int size = 1;
        if (language != null) size += PacketIO.stringSize(language);
