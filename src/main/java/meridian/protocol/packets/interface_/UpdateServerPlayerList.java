...
public class UpdateServerPlayerList implements Packet, ToClientPacket {
    ...
    public static final int MAX_SIZE = 131072005;
    ...
    @Nonnull public ServerPlayerListUpdate[] players = new ServerPlayerListUpdate[0];

    public UpdateServerPlayerList() {
    }

    public UpdateServerPlayerList(@Nonnull ServerPlayerListUpdate[] players) {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("UpdateServerPlayerList", offset, (int) mem.byteSize());
        long needed = (long) offset + 0;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("UpdateServerPlayerList", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static ServerPlayerListUpdate[] getPlayers(MemorySegment mem, int offset) {
        ...
        if (off + lenOffset + (long) len * 32 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Players", (int) java.lang.Math.min(off + lenOffset + (long) len * 32, Integer.MAX_VALUE), (int) mem.byteSize());
        off += lenOffset;
        var data = new ServerPlayerListUpdate[len];
        for (var i = 0; i < len; i++) {
    ...
    /**
     * Decodes one UpdateServerPlayerList and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static UpdateServerPlayerList toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var varBase = offset + 0;
        var varPos = 0;
        ServerPlayerListUpdate[] v0;
        {
            ...
            if (off + lenOffset + (long) len * 32 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Players", (int) java.lang.Math.min(off + lenOffset + (long) len * 32, Integer.MAX_VALUE), (int) mem.byteSize());
            off += lenOffset;
            v0 = new ServerPlayerListUpdate[len];
            for (var i = 0; i < len; i++) {
                v0[i] = ServerPlayerListUpdate.toObject(mem, off + i * 32);
            }
            varPos = off + len * 32 - varBase;
        }
        var result = new UpdateServerPlayerList(
            v0
        );
    ...
    public int computeSize() {
        int size = 0;
        size += VarInt.size(players.length) + players.length * 32;
    ...
    public UpdateServerPlayerList clone() {
        ...
        copy.players = java.util.Arrays.stream(this.players).map(e -> e.clone()).toArray(ServerPlayerListUpdate[]::new);
        return copy;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateServerPlayerList other)) return false;
