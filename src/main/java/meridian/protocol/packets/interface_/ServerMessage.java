...
public class ServerMessage implements Packet, ToClientPacket {
    ...
    @Nonnull public ChatType type = ChatType.Chat;
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("ServerMessage", offset, (int) mem.byteSize());
        long needed = (long) offset + 2;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("ServerMessage", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    /**
     * Decodes one ServerMessage and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static ServerMessage toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        var result = new ServerMessage(
            ChatType.fromValue(mem.get(PacketIO.PROTO_BYTE, offset + 1)),
            v1
        );
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ServerMessage other)) return false;
