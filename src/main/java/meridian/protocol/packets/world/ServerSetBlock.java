...
public class ServerSetBlock implements Packet, ToClientPacket {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("ServerSetBlock", offset, (int) mem.byteSize());
        long needed = (long) offset + 19;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("ServerSetBlock", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    /**
     * Decodes one ServerSetBlock and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static ServerSetBlock toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var result = new ServerSetBlock(
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ServerSetBlock other)) return false;
        return this.x == other.x && this.y == other.y && this.z == other.z && this.blockId == other.blockId && this.filler == other.filler && this.rotation == other.rotation;
    }
