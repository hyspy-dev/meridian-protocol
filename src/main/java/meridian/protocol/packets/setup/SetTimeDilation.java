...
public class SetTimeDilation implements Packet, ToClientPacket {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("SetTimeDilation", offset, (int) mem.byteSize());
        long needed = (long) offset + 4;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("SetTimeDilation", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static float getTimeDilation(MemorySegment mem, int offset) {
        return PacketIO.requireFinite(mem.get(PacketIO.PROTO_FLOAT, offset + 0), "TimeDilation");
    }
    ...
    /**
     * Decodes one SetTimeDilation and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static SetTimeDilation toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var result = new SetTimeDilation(
            PacketIO.requireFinite(mem.get(PacketIO.PROTO_FLOAT, offset + 0), "TimeDilation")
        );
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        PacketIO.requireFinite(this.timeDilation, "TimeDilation"); mem.set(PacketIO.PROTO_FLOAT, offset + 0, this.timeDilation);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof SetTimeDilation other)) return false;
