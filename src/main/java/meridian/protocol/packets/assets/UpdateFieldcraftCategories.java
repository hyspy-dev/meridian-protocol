...
public class UpdateFieldcraftCategories implements Packet, ToClientPacket {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("UpdateFieldcraftCategories", offset, (int) mem.byteSize());
        long needed = (long) offset + 2;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("UpdateFieldcraftCategories", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    /**
     * Decodes one UpdateFieldcraftCategories and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static UpdateFieldcraftCategories toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        var result = new UpdateFieldcraftCategories(
...
size += VarInt.size(itemCategories.length) + itemCategoriesSize;
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateFieldcraftCategories other)) return false;
