...
public class UpdateAudioStates implements Packet, ToClientPacket {
    ...
    public static java.util.Map<Integer, AudioState> getAudioStates(MemorySegment mem, int offset) {
        ...
        off += (int) (packed >>> 32);
        if (off + (long) len * 31 > mem.byteSize()) throw ProtocolException.bufferTooSmall("AudioStates", (int) java.lang.Math.min(off + (long) len * 31, Integer.MAX_VALUE), (int) mem.byteSize());
