...
public class UpdateRootInteractions implements Packet, ToClientPacket {
    ...
    public int maxId;
    @Nullable public java.util.Map<Integer, RootInteraction> interactions;

    public UpdateRootInteractions() {
    }

    public UpdateRootInteractions(@Nonnull UpdateType type, int maxId, @Nullable java.util.Map<Integer, RootInteraction> interactions) {
        this.type = type;
        this.maxId = maxId;
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("UpdateRootInteractions", offset, (int) mem.byteSize());
        long needed = (long) offset + 6;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("UpdateRootInteractions", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    @Nullable
    public static java.util.Map<Integer, RootInteraction> getInteractions(MemorySegment mem) {
        return getInteractions(mem, 0);
    }
    
    @Nullable
    public static java.util.Map<Integer, RootInteraction> getInteractions(MemorySegment mem, int offset) {
        ...
        off += (int) (packed >>> 32);
        if (off + (long) len * 38 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Interactions", (int) java.lang.Math.min(off + (long) len * 38, Integer.MAX_VALUE), (int) mem.byteSize());
        java.util.Map<Integer, RootInteraction> data = new HashMap<>(len);
        for (var i = 0; i < len; i++) {
            var key = mem.get(PacketIO.PROTO_INT, off);
                off += 4;
            var value = RootInteraction.toObject(mem, off, walkCursor);
    ...
    public static UpdateRootInteractions toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static UpdateRootInteractions toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one UpdateRootInteractions and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static UpdateRootInteractions toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var varBase = offset + 6;
        var varPos = 0;
        var walkCursor = cursor != null ? cursor : new ReadCursor();
        java.util.Map<Integer, RootInteraction> v2 = null;
        ...
        var result = new UpdateRootInteractions(
...
for (var kvp : interactions.entrySet()) interactionsSize += 4 + kvp.getValue().computeSize();
size += VarInt.size(interactions.size()) + interactionsSize;
    ...
    public UpdateRootInteractions clone() {
        ...
        if (this.interactions != null) {
            java.util.Map<Integer, RootInteraction> m = new java.util.HashMap<>();
            for (var e : this.interactions.entrySet()) { m.put(e.getKey(), e.getValue().clone()); }
            copy.interactions = m;
        }
        return copy;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateRootInteractions other)) return false;
