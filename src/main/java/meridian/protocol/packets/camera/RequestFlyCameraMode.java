...
public class RequestFlyCameraMode implements Packet, ToServerPacket {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("RequestFlyCameraMode", offset, (int) mem.byteSize());
        long needed = (long) offset + 1;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("RequestFlyCameraMode", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static RequestFlyCameraMode toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static RequestFlyCameraMode toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one RequestFlyCameraMode and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static RequestFlyCameraMode toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        // Checking the whole fixed block up front lets the JIT elide the per-field bound checks.
        requireBounds(mem, offset);
        var result = new RequestFlyCameraMode(
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof RequestFlyCameraMode other)) return false;
