...
public class OrConnectedBlockRule extends ConnectedBlockRule {
    ...
    public OrConnectedBlockRule() {
    }

    public OrConnectedBlockRule(@Nullable ConnectedBlockRule[] rules) {
        this.rules = rules;
    }

    public OrConnectedBlockRule(@Nonnull OrConnectedBlockRule other) {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("OrConnectedBlockRule", offset, (int) mem.byteSize());
        long needed = (long) offset + 1;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("OrConnectedBlockRule", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static OrConnectedBlockRule toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static OrConnectedBlockRule toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one OrConnectedBlockRule and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static OrConnectedBlockRule toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        var result = new OrConnectedBlockRule(
            v0
        );
...
size += VarInt.size(rules.length) + rulesSize;
    ...
    public OrConnectedBlockRule clone() {
        OrConnectedBlockRule copy = new OrConnectedBlockRule();
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof OrConnectedBlockRule other)) return false;
