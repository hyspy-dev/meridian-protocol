...
public class ReplaceInteraction extends Interaction {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("ReplaceInteraction", offset, (int) mem.byteSize());
        long needed = (long) offset + 42;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("ReplaceInteraction", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static int getDefaultValue(MemorySegment mem) {
        return getDefaultValue(mem, 0);
    }
    
    public static int getDefaultValue(MemorySegment mem, int offset) {
        return mem.get(PacketIO.PROTO_INT, offset + 14);
    }
    
    @Nullable
    public static String getVariable(MemorySegment mem) {
        return getVariable(mem, 0);
    }
    
    @Nullable
    public static String getVariable(MemorySegment mem, int offset) {
        return hasVariable(mem, offset) ? PacketIO.readVarString("Variable", mem, offset + getValidatedOffset(mem, offset, 38, 42, "Variable"), 4096000): null;
    }
    ...
    public static boolean hasVariable(MemorySegment mem, int offset) {
        var b = mem.get(PacketIO.PROTO_BYTE, offset + 0);
        return (b & 0x10) != 0;
    }
    ...
    public static ReplaceInteraction toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static ReplaceInteraction toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one ReplaceInteraction and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static ReplaceInteraction toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        String v10 = null;
        if (hasVariable(mem, offset)) {
            requireSlot(mem, offset + 38, varPos, "Variable");
            var off = varBase + varPos;
            var sp = VarInt.getWithLength(mem, off);
            v10 = PacketIO.readVarString("Variable", mem, off, 0, 4096000, sp);
            varPos += (int) sp + (int) (sp >>> 32);
        } else {
            requireSlot(mem, offset + 38, -1, "Variable");
        }
        var result = new ReplaceInteraction(
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        if (this.variable != null) nullBits |= 0x10;
        ...
        mem.set(PacketIO.PROTO_INT, offset + 14, this.defaultValue);
        ...
        if (this.variable != null) {
            ...
            varOffset += PacketIO.writeVarString(mem, varOffset, this.variable, 4096000);
        } else {
    ...
    if (variable != null) size += PacketIO.stringSize(variable);
    ...
    public int hashCode() {
        ...
        result = 31 * result + Integer.hashCode(defaultValue);
        result = 31 * result + java.util.Objects.hashCode(variable);
        return result;
    }
