...
public class SerialInteraction extends Interaction {
    ...
    @Nonnull public int[] serialInteractions = new int[0];

    public SerialInteraction() {
    }

    public SerialInteraction(@Nonnull WaitForDataFrom waitForDataFrom, @Nullable InteractionEffects effects, float horizontalSpeedMultiplier, float runTime, @Nonnull InteractionItemChangeBehavior onItemChangeBehavior, @Nullable java.util.Map<GameMode, InteractionSettings> settings, @Nonnull InteractionRules rules, @Nullable int[] tags, @Nullable InteractionCameraSettings camera, @Nonnull int[] serialInteractions) {
        ...
        this.tags = tags;
        this.camera = camera;
        this.serialInteractions = serialInteractions;
    }
    ...
    public SerialInteraction(@Nonnull SerialInteraction other) {
        ...
        this.serialInteractions = other.serialInteractions;
    }
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("SerialInteraction", offset, (int) mem.byteSize());
        long needed = (long) offset + 38;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("SerialInteraction", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static InteractionEffects getEffects(MemorySegment mem, int offset) {
        return hasEffects(mem, offset) ? InteractionEffects.toObject(mem, offset + getValidatedOffset(mem, offset, 14, 38, "Effects")): null;
    }
    ...
    public static java.util.Map<GameMode, InteractionSettings> getSettings(MemorySegment mem, int offset) {
        ...
        var off = offset + getValidatedOffset(mem, offset, 18, 38, "Settings");
    ...
    public static InteractionRules getRules(MemorySegment mem, int offset) {
        return InteractionRules.toObject(mem, offset + getValidatedOffset(mem, offset, 22, 38, "Rules"));
    }
    ...
    public static int[] getTags(MemorySegment mem, int offset) {
        ...
        var off = offset + getValidatedOffset(mem, offset, 26, 38, "Tags");
        var packed = VarInt.getWithLength(mem, off);
    ...
    public static InteractionCameraSettings getCamera(MemorySegment mem, int offset) {
        return hasCamera(mem, offset) ? InteractionCameraSettings.toObject(mem, offset + getValidatedOffset(mem, offset, 30, 38, "Camera")): null;
    }
    
    public static int[] getSerialInteractions(MemorySegment mem) {
        return getSerialInteractions(mem, 0);
    }
    
    public static int[] getSerialInteractions(MemorySegment mem, int offset) {
        var off = offset + getValidatedOffset(mem, offset, 34, 38, "SerialInteractions");
        var packed = VarInt.getWithLength(mem, off);
        if (packed == -1L) throw ProtocolException.invalidVarInt("SerialInteractions");
        var len = (int) packed;
        if (len > 4096000) throw ProtocolException.arrayTooLong("SerialInteractions", len, 4096000);
        var lenOffset = (int) (packed >>> 32);
        if (off + lenOffset + (long) len * 4 > mem.byteSize()) throw ProtocolException.bufferTooSmall("SerialInteractions", (int) java.lang.Math.min(off + lenOffset + (long) len * 4, Integer.MAX_VALUE), (int) mem.byteSize());
        off += lenOffset;
        var data = new int[len];
        MemorySegment.copy(mem, PacketIO.PROTO_INT, off, data, 0, len);
        return data;
    ...
    public static SerialInteraction toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static SerialInteraction toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one SerialInteraction and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static SerialInteraction toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        if (hasEffects(mem, offset)) {
            requireSlot(mem, offset + 14, varPos, "Effects");
            v1 = InteractionEffects.toObject(mem, varBase + varPos, walkCursor);
            varPos = walkCursor.position - varBase;
        } else {
            requireSlot(mem, offset + 14, -1, "Effects");
        }
        ...
        if (hasCamera(mem, offset)) {
            requireSlot(mem, offset + 30, varPos, "Camera");
            v8 = InteractionCameraSettings.toObject(mem, varBase + varPos, walkCursor);
            varPos = walkCursor.position - varBase;
        } else {
            requireSlot(mem, offset + 30, -1, "Camera");
        }
        
        int[] v9;
        requireSlot(mem, offset + 34, varPos, "SerialInteractions");
        {
            var off = varBase + varPos;
            var packed = VarInt.getWithLength(mem, off);
            if (packed == -1L) throw ProtocolException.invalidVarInt("SerialInteractions");
        ...
        var result = new SerialInteraction(
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        if (this.effects != null) {
            mem.set(PacketIO.PROTO_INT, offset + 14, varOffset - offset - 38);
        ...
        if (serialInteractions.length > 4096000) throw ProtocolException.arrayTooLong("SerialInteractions", serialInteractions.length, 4096000);
        varOffset += VarInt.set(mem, varOffset, this.serialInteractions.length);
        
        MemorySegment.copy(this.serialInteractions, 0, mem, PacketIO.PROTO_INT, varOffset, this.serialInteractions.length);
        varOffset += this.serialInteractions.length * 4;
    ...
    size += VarInt.size(serialInteractions.length) + serialInteractions.length * 4;

        return size;
    }

    public SerialInteraction clone() {
        SerialInteraction copy = new SerialInteraction();
        ...
        copy.serialInteractions = java.util.Arrays.copyOf(this.serialInteractions, this.serialInteractions.length);
        return copy;
    }
    ...
    public int hashCode() {
        ...
        result = 31 * result + java.util.Arrays.hashCode(serialInteractions);
        return result;
    }
