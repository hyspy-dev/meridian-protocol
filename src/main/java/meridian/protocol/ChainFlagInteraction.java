...
public class ChainFlagInteraction extends SimpleInteraction {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("ChainFlagInteraction", offset, (int) mem.byteSize());
        long needed = (long) offset + 50;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("ChainFlagInteraction", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static InteractionEffects getEffects(MemorySegment mem, int offset) {
        return hasEffects(mem, offset) ? InteractionEffects.toObject(mem, offset + getValidatedOffset(mem, offset, 22, 50, "Effects")): null;
    }
    ...
    public static java.util.Map<GameMode, InteractionSettings> getSettings(MemorySegment mem, int offset) {
        ...
        var off = offset + getValidatedOffset(mem, offset, 26, 50, "Settings");
    ...
    public static InteractionRules getRules(MemorySegment mem, int offset) {
        return InteractionRules.toObject(mem, offset + getValidatedOffset(mem, offset, 30, 50, "Rules"));
    }
    ...
    public static int[] getTags(MemorySegment mem, int offset) {
        ...
        var off = offset + getValidatedOffset(mem, offset, 34, 50, "Tags");
        var packed = VarInt.getWithLength(mem, off);
    ...
    public static InteractionCameraSettings getCamera(MemorySegment mem, int offset) {
        return hasCamera(mem, offset) ? InteractionCameraSettings.toObject(mem, offset + getValidatedOffset(mem, offset, 38, 50, "Camera")): null;
    }
    ...
    public static String getChainId(MemorySegment mem, int offset) {
        return PacketIO.readVarString("ChainId", mem, offset + getValidatedOffset(mem, offset, 42, 50, "ChainId"), 4096000);
    }
    
    public static String getFlag(MemorySegment mem) {
        return getFlag(mem, 0);
    }
    
    public static String getFlag(MemorySegment mem, int offset) {
        return PacketIO.readVarString("Flag", mem, offset + getValidatedOffset(mem, offset, 46, 50, "Flag"), 4096000);
    }
    ...
    public static ChainFlagInteraction toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static ChainFlagInteraction toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one ChainFlagInteraction and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static ChainFlagInteraction toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        if (hasSettings(mem, offset)) {
            ...
            requireSlot(mem, offset + 26, -1, "Settings");
        }
        
        InteractionRules v6;
        requireSlot(mem, offset + 30, varPos, "Rules");
        {
        ...
        if (hasCamera(mem, offset)) {
            requireSlot(mem, offset + 38, varPos, "Camera");
            v8 = InteractionCameraSettings.toObject(mem, varBase + varPos, walkCursor);
            varPos = walkCursor.position - varBase;
        } else {
            requireSlot(mem, offset + 38, -1, "Camera");
        }
        
        String v11;
        requireSlot(mem, offset + 42, varPos, "ChainId");
        {
            var off = varBase + varPos;
            var sp = VarInt.getWithLength(mem, off);
            v11 = PacketIO.readVarString("ChainId", mem, off, 0, 4096000, sp);
            varPos += (int) sp + (int) (sp >>> 32);
        }
        
        String v12;
        requireSlot(mem, offset + 46, varPos, "Flag");
        {
            var off = varBase + varPos;
            var sp = VarInt.getWithLength(mem, off);
            v12 = PacketIO.readVarString("Flag", mem, off, 0, 4096000, sp);
            varPos += (int) sp + (int) (sp >>> 32);
        }
        var result = new ChainFlagInteraction(
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        varOffset += PacketIO.writeVarString(mem, varOffset, this.flag, 4096000);
    ...
    size += PacketIO.stringSize(chainId);
    size += PacketIO.stringSize(flag);
    ...
    public int hashCode() {
        ...
        result = 31 * result + java.util.Objects.hashCode(flag);
        return result;
    }
