...
public class StringParamValue extends ParamValue {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("StringParamValue", offset, (int) mem.byteSize());
        long needed = (long) offset + 1;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("StringParamValue", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static String getValue(MemorySegment mem, int offset) {
        return hasValue(mem, offset) ? PacketIO.readVarString("Value", mem, offset + 1, 4096000): null;
    }
    ...
    /**
     * Decodes one StringParamValue and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static StringParamValue toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        var result = new StringParamValue(
            v0
        );
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        byte nullBits;
        nullBits = 0;
        if (this.value != null) nullBits |= 0x01;
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof StringParamValue other)) return false;
