...
public class ChangeStateInteraction extends SimpleBlockInteraction {
    ...
    @Nullable
    public static java.util.Map<String, String> getStateChanges(MemorySegment mem) {
    ...
    public static java.util.Map<String, String> getStateChanges(MemorySegment mem, int offset) {
        ...
        var len = (int) packed;
        if (len > 4096000) throw ProtocolException.dictionaryTooLarge("StateChanges", len, 4096000);
        
        off += (int) (packed >>> 32);
        if (off + (long) len * 2 > mem.byteSize()) throw ProtocolException.bufferTooSmall("StateChanges", (int) java.lang.Math.min(off + (long) len * 2, Integer.MAX_VALUE), (int) mem.byteSize());
    ...
    /**
     * Decodes one ChangeStateInteraction and reports the end of its encoding through the cursor.
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        if (this.stateChanges != null) nullBits |= 0x10;
...
for (var kvp : stateChanges.entrySet()) stateChangesSize += PacketIO.stringSize(kvp.getKey()) + PacketIO.stringSize(kvp.getValue());
size += VarInt.size(stateChanges.size()) + stateChangesSize;
    }
