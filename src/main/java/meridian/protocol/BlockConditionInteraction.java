...
public class BlockConditionInteraction extends SimpleBlockInteraction {
    ...
    @Nullable public BlockMatcher[] matchers;
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("BlockConditionInteraction", offset, (int) mem.byteSize());
        long needed = (long) offset + 47;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("BlockConditionInteraction", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    @Nullable
    public static BlockMatcher[] getMatchers(MemorySegment mem) {
        return getMatchers(mem, 0);
    }
    
    @Nullable
    public static BlockMatcher[] getMatchers(MemorySegment mem, int offset) {
        if (!hasMatchers(mem, offset)) return null;
        var walkCursor = new ReadCursor();
        var off = offset + getValidatedOffset(mem, offset, 43, 47, "Matchers");
        var packed = VarInt.getWithLength(mem, off);
        ...
        var lenOffset = (int) (packed >>> 32);
        if (off + lenOffset + (long) len * 3 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Matchers", (int) java.lang.Math.min(off + lenOffset + (long) len * 3, Integer.MAX_VALUE), (int) mem.byteSize());
        off += lenOffset;
        var data = new BlockMatcher[len];
        for (var i = 0; i < len; i++) {
            data[i] = BlockMatcher.toObject(mem, off, walkCursor);
    ...
    public static BlockConditionInteraction toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static BlockConditionInteraction toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one BlockConditionInteraction and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static BlockConditionInteraction toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        BlockMatcher[] v12 = null;
        if (hasMatchers(mem, offset)) {
            requireSlot(mem, offset + 43, varPos, "Matchers");
            var off = varBase + varPos;
            var packed = VarInt.getWithLength(mem, off);
            if (packed == -1L) throw ProtocolException.invalidVarInt("Matchers");
            var len = (int) packed;
            ...
            if (off + lenOffset + (long) len * 3 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Matchers", (int) java.lang.Math.min(off + lenOffset + (long) len * 3, Integer.MAX_VALUE), (int) mem.byteSize());
            off += lenOffset;
            v12 = new BlockMatcher[len];
            for (var i = 0; i < len; i++) {
                v12[i] = BlockMatcher.toObject(mem, off, walkCursor);
                off = walkCursor.position;
            }
            varPos = off - varBase;
        } else {
            requireSlot(mem, offset + 43, -1, "Matchers");
        }
        var result = new BlockConditionInteraction(
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        if (this.matchers != null) nullBits |= 0x10;
    ...
    if (matchers != null) {
        int matchersSize = 0;
for (var elem : matchers) matchersSize += elem.computeSize();
size += VarInt.size(matchers.length) + matchersSize;
    }
    ...
    public BlockConditionInteraction clone() {
        ...
        copy.matchers = this.matchers != null ? java.util.Arrays.stream(this.matchers).map(e -> e.clone()).toArray(BlockMatcher[]::new) : null;
        return copy;
    }
