...
public class ParallelInteraction extends Interaction {
    ...
    @Nonnull public int[] next = new int[0];

    public ParallelInteraction() {
    }

    public ParallelInteraction(@Nonnull WaitForDataFrom waitForDataFrom, @Nullable InteractionEffects effects, float horizontalSpeedMultiplier, float runTime, @Nonnull InteractionItemChangeBehavior onItemChangeBehavior, @Nullable java.util.Map<GameMode, InteractionSettings> settings, @Nonnull InteractionRules rules, @Nullable int[] tags, @Nullable InteractionCameraSettings camera, @Nonnull int[] next) {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("ParallelInteraction", offset, (int) mem.byteSize());
        long needed = (long) offset + 38;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("ParallelInteraction", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static int[] getNext(MemorySegment mem, int offset) {
        var off = offset + getValidatedOffset(mem, offset, 34, 38, "Next");
        var packed = VarInt.getWithLength(mem, off);
        if (packed == -1L) throw ProtocolException.invalidVarInt("Next");
        var len = (int) packed;
        if (len > 4096000) throw ProtocolException.arrayTooLong("Next", len, 4096000);
        var lenOffset = (int) (packed >>> 32);
        if (off + lenOffset + (long) len * 4 > mem.byteSize()) throw ProtocolException.bufferTooSmall("Next", (int) java.lang.Math.min(off + lenOffset + (long) len * 4, Integer.MAX_VALUE), (int) mem.byteSize());
        off += lenOffset;
        var data = new int[len];
        MemorySegment.copy(mem, PacketIO.PROTO_INT, off, data, 0, len);
        return data;
    ...
    public static ParallelInteraction toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static ParallelInteraction toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one ParallelInteraction and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static ParallelInteraction toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        int[] v9;
        requireSlot(mem, offset + 34, varPos, "Next");
        {
            var off = varBase + varPos;
            var packed = VarInt.getWithLength(mem, off);
            if (packed == -1L) throw ProtocolException.invalidVarInt("Next");
        ...
        var result = new ParallelInteraction(
    ...
    public int serialize(@Nonnull MemorySegment mem, int offset) {
        ...
        if (next.length > 4096000) throw ProtocolException.arrayTooLong("Next", next.length, 4096000);
        varOffset += VarInt.set(mem, varOffset, this.next.length);
        
        MemorySegment.copy(this.next, 0, mem, PacketIO.PROTO_INT, varOffset, this.next.length);
        varOffset += this.next.length * 4;
    ...
    size += VarInt.size(next.length) + next.length * 4;

        return size;
    }

    public ParallelInteraction clone() {
        ParallelInteraction copy = new ParallelInteraction();
        ...
        copy.next = java.util.Arrays.copyOf(this.next, this.next.length);
        return copy;
    }
    ...
    public int hashCode() {
        ...
        result = 31 * result + java.util.Arrays.hashCode(next);
        return result;
    }
