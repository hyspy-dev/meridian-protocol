...
public class ResetCooldownInteraction extends SimpleInteraction {
    ...
    public static void requireBounds(MemorySegment mem, int offset) {
        if (offset < 0) throw ProtocolException.invalidOffset("ResetCooldownInteraction", offset, (int) mem.byteSize());
        long needed = (long) offset + 46;
        if (needed > mem.byteSize()) throw ProtocolException.bufferTooSmall("ResetCooldownInteraction", (int) java.lang.Math.min(needed, Integer.MAX_VALUE), (int) mem.byteSize());
    }
    ...
    public static java.util.Map<GameMode, InteractionSettings> getSettings(MemorySegment mem, int offset) {
        ...
        var off = offset + getValidatedOffset(mem, offset, 26, 46, "Settings");
    ...
    public static InteractionRules getRules(MemorySegment mem, int offset) {
        return InteractionRules.toObject(mem, offset + getValidatedOffset(mem, offset, 30, 46, "Rules"));
    }
    ...
    public static int[] getTags(MemorySegment mem, int offset) {
        ...
        var off = offset + getValidatedOffset(mem, offset, 34, 46, "Tags");
        var packed = VarInt.getWithLength(mem, off);
    ...
    public static InteractionCooldown getCooldown(MemorySegment mem, int offset) {
        return hasCooldown(mem, offset) ? InteractionCooldown.toObject(mem, offset + getValidatedOffset(mem, offset, 42, 46, "Cooldown")): null;
    }
    ...
    public static ResetCooldownInteraction toObject(MemorySegment mem) {
        return toObject(mem, 0, null);
    }
    
    public static ResetCooldownInteraction toObject(MemorySegment mem, int offset) {
        return toObject(mem, offset, null);
    }
    
    /**
     * Decodes one ResetCooldownInteraction and reports the end of its encoding through the cursor.
     * The variable block is decoded in field order against a running position, and each
     * offset slot must name that position, so the fields decoded are the bytes walked.
     */
    public static ResetCooldownInteraction toObject(MemorySegment mem, int offset, @Nullable ReadCursor cursor) {
        ...
        if (hasCooldown(mem, offset)) {
            requireSlot(mem, offset + 42, varPos, "Cooldown");
            v11 = InteractionCooldown.toObject(mem, varBase + varPos, walkCursor);
            varPos = walkCursor.position - varBase;
        } else {
            requireSlot(mem, offset + 42, -1, "Cooldown");
        }
        var result = new ResetCooldownInteraction(
    ...
    public ResetCooldownInteraction clone() {
        ResetCooldownInteraction copy = new ResetCooldownInteraction();
